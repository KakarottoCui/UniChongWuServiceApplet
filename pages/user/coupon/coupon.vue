		...
		<view>
			...
			<view class="coupon-list">
				...
				<view
					...
					:style="{ marginTop: state === 3 ? '50rpx' : 0 }"
				>
						...
						<view class="carrier">
							<view class="left">
								...
								<view class="term" v-if="state !== 2">
									<text v-if="row.term_of_validity_type === '1'">领取之日起{{ row.fixed_term }}天内有效</text>
									<text v-else-if="row.start_time === row.end_time">限{{ row.start_time | time }}当日使用</text>
									<text v-else>使用时间：{{ row.start_time | time }} - {{ row.end_time | time }}</text>
								</view>
...
.sub-list {
	...
	.row {
		...
		border-radius: 8rpx;
		// box-shadow: 0rpx 0 10rpx rgba(0,0,0,0.1);
		align-items: center;
		position: relative;
		overflow: hidden;
		z-index: 4;
